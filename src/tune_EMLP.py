"""
Hyperparameter tuning for the EmbeddingMLP ADR prediction model using Optuna.

Tunes embedding dimension, hidden layer width/depth, dropout, learning rate,
focal loss parameters, and batch size on the min1000 expanded dataset.

Progress is saved to an SQLite database so runs can be stopped and resumed.
Each invocation adds --trials more trials to the same study.

Usage:
    python src/tune_EMLP.py                    # run 20 trials (resumable)
    python src/tune_EMLP.py --trials 50        # run 50 trials
    python src/tune_EMLP.py --dataset expanded # full expanded dataset
    python src/tune_EMLP.py --reset            # delete saved study and start fresh
"""
...
def load_dataset(dataset: str):
    """Load train/val splits and encoder vocab sizes."""
    ...
    with open(d / "label_encoders.pkl", "rb") as f:
    ...
    return X_train, y_train, X_val, y_val, num_drugs, num_routes, num_dose_units, num_numerical
...
def run_trial(
    ...
    """Train one EmbeddingMLP configuration and return validation AUROC."""
    ...
    hidden_dims = [max(32, width // (2 ** i)) for i in range(depth)]

    print(f"\n{'='*60}")
    print(f"Trial {trial.number}  |  emb_dim={embedding_dim}  lr={lr:.5f}  "
          f"dropout={dropout}  alpha={focal_alpha}  gamma={focal_gamma}")
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        model.train()
        epoch_loss = 0.0
        for d_idx, r_idx, du_idx, numerical, y_batch in train_loader:
        ...
        vpreds, vtargets = [], []
        with torch.no_grad():
            for d_idx, r_idx, du_idx, numerical, y_batch in val_loader:
                d_idx, r_idx, du_idx = d_idx.to(device), r_idx.to(device), du_idx.to(device)
...
def tune(n_trials: int = 20, dataset: str = "min1000_expanded", reset: bool = False) -> None:
    ...
    db_path    = results_dir / f"tuning_emlp_{dataset}.db"
    storage    = f"sqlite:///{db_path}"
    study_name = f"emlp_{dataset}"
    ...
    X_train, y_train, X_val, y_val, num_drugs, num_routes, num_dose_units, num_numerical = \
    ...
    study.optimize(
        lambda trial: run_trial(
            ...
            num_drugs, num_routes, num_dose_units, num_numerical, device,
        ),
        n_trials=n_trials,
    ...
    out_path = results_dir / f"tuning_emlp_{dataset}.json"
    ...
    print("\nTop 5 trials:")
    print(f"  {'#':<5} {'AUROC':<10} {'emb_dim':<10} {'hidden':<20} {'lr':<10} "
          f"{'dropout':<10} {'alpha':<8} {'gamma':<8} {'batch'}")
    for t in top5:
        p = t.params
        hidden = [max(32, p['width'] // (2**i)) for i in range(p['depth'])]
        print(f"  {t.number:<5} {t.value:<10.4f} {p['embedding_dim']:<10} "
              f"{str(hidden):<20} {p['lr']:<10.5f} {p['dropout']:<10.1f} "
              f"{p['focal_alpha']:<8.2f} {p['focal_gamma']:<8.1f} {p['batch_size']}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Optuna hyperparameter tuning for EmbeddingMLP")
